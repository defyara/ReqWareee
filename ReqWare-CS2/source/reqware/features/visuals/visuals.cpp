...
void Visuals::esp() {
    ...
    for (const auto& Player : cached_players)
    {
        ...
        ImU32 espCol = ImColor(Config::espColor);
        ...
        if (Config::espLine) {
            // Получаем экранные координаты ног и головы игрока
            Vector_t feetScreen, headScreen;
            if (!viewMatrix.WorldToScreen(Player.position, feetScreen) ||
                !viewMatrix.WorldToScreen(Player.position + Player.viewOffset, headScreen))
                continue; // Пропускаем, если игрок не на экране

            // Вычисляем центр бокса игрока
            float boxHeight = (feetScreen.y - headScreen.y) * 1.3f;
            float boxWidth = boxHeight / 2.0f;
            float boxCenterX = feetScreen.x;
            float boxCenterY = headScreen.y + (feetScreen.y - headScreen.y) * 0.5f;

            ImVec2 lineStart(ImGui::GetIO().DisplaySize.x * 0.5f, ImGui::GetIO().DisplaySize.y);

            ImVec2 lineEnd(boxCenterX, boxCenterY);

            drawList->AddLine(
                lineStart,
                lineEnd,
                ImColor(Config::lineColor), // Цвет из конфига
                Config::lineThickness // Толщина из конфига
            );
        }
